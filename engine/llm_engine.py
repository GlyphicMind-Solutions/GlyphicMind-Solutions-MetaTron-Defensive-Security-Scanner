# ./MetaTron/engine/llm_engine.py
# MetaTron Security Tool - LLM Engine
...
class LLMEngine:
    ...
    def __init__(self, root: Path):
        self.root = Path(root)
        self.manifest_path = self.root / "models" / "manifest.yaml"
    ...
    def generate(self, prompt: str, model_key: Optional[str] = None, max_tokens: int = 2048) -> str:
        ...
        The prompt is assumed to be fully formatted (templates already applied).
        """
        if model_key is None:
            model_key = self.default_key
